...
export class LineManager {
  ...
  // 添加折线
  async addPolyline(map: any, options: IUnifiedPolylineOptions): Promise<any> {
  ...
  addPolylineSync(map: any, options: IUnifiedPolylineOptions): any {
    // const { Polyline } = await this.loader.importLibrary("maps");
    const Polyline = map.PolylineMapKit;
    let lineOptions = { ...options } as any;
    if (options?.showDirection) {
      lineOptions.icons = [
        {
   ...
   * 支持倍速播放、时间点跳转等高级功能
   */
  async animateTimeBasedPath(
    map: any,
    options: ITimeBasedPathAnimateOptions
  ): Promise<ITimeBasedPathAnimationController> {
    ...
    return new Promise((resolve) => {
      ...
      // 创建路径线
      const fullPath = options.path.map((p) => ({ lat: p.lat, lng: p.lng }));
      const fullPathLine = new Polyline({
        ...
        icons: options.pathStyle?.showDirection
          ? [
              {
                icon: {
                  path: google.maps.SymbolPath.FORWARD_CLOSED_ARROW,
      ...
      // 已走过的路径线
      let passedPath: any[] = [];
      const passedPathLine = new Polyline({
        path: [],
        geodesic: true,
        ...
        icons: options.passedStyle?.showDirection
          ? [
              {
                icon: {
                  path: google.maps.SymbolPath.FORWARD_CLOSED_ARROW,
                  scale: 4,
                  strokeColor: options.pathStyle?.strokeColor || "#00AA00",
                  strokeWeight: 1,
                  fillOpacity: 1,
                },
                repeat: "10%",
      ...
      if (options.markerStyle?.visible !== false) {
        ...
        if (options.markerStyle?.icon) {
          if (typeof options.markerStyle.icon === "string") {
            ...
            markerOptions.icon = {
              url: options.markerStyle.icon.url,
              scaledSize: options.markerStyle.icon.size
                ? new google.maps.Size(
                    options.markerStyle.icon.size[0],
                    options.markerStyle.icon.size[1]
                  )
      ...
      const getPositionAtTime = (time: number) => {
        ...
        if (time >= maxTime)
          return {
            point: options.path[options.path.length - 1],
            index: options.path.length - 1,
          };
      ...
      const updatePosition = (time: number) => {
        ...
        // 触发回调
        options.callbacks?.onTimeUpdate?.(
          time,
        ...
        if (Math.abs(index - Math.round(index)) < 0.01) {
          ...
          if (pointIndex < options.path.length) {
            options.callbacks?.onReachPoint?.(
              options.path[pointIndex],
              pointIndex
            );
          }
        }
      ...
      const animate = () => {
        ...
        const currentTime =
          minTime + (elapsed / totalDuration) * (maxTime - minTime);
      ...
      const controller: ITimeBasedPathAnimationController = {
        start() {
          ...
          animationState.startTime =
            Date.now() -
            (((animationState.currentTime - minTime) / (maxTime - minTime)) *
              totalDuration) /
              config.speed;
        ...
        seekToPoint(index: number) {
          const clampedIndex = Math.max(
            0,
            Math.min(options.path.length - 1, index)
          );
        ...
        getState() {
          return {
            ...
            progress:
              (animationState.currentTime - minTime) / (maxTime - minTime),
            currentPointIndex: getPositionAtTime(animationState.currentTime)
              .index,
            speed: config.speed,
          };
        },
        ...
        destroy() {
          ...
          if (fullPathLine) {
            fullPathLine.setMap(null);
          }
          if (passedPathLine) {
            passedPathLine.setMap(null);
          }
        },
      };
  ...
  animateTimeBasedPathSync(
    map: any,
    options: ITimeBasedPathAnimateOptions
  ): ITimeBasedPathAnimationController {
    // const { Marker } = await this.loader.importLibrary("marker");
    // const { Polyline } = await this.loader.importLibrary("maps");
    const Marker = map.MarkerMapKit;
    const Polyline = map.PolylineMapKit;
    // 初始化配置
    const config = {
      speed: options.speed || 1.0,
