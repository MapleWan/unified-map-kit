...
export class LineManager {
   ...
   * 支持倍速播放、时间点跳转等高级功能
   */
  async animateTimeBasedPath(map: any, options: ITimeBasedPathAnimateOptions): Promise<ITimeBasedPathAnimationController> {
    return new Promise((resolve) => {
      ...
      // 已走过的路径线
      let passedPath: any[] = [];
      const passedPathLine = new HWMapJsSDK.HWPolyline({
        path: [],
      ...
      if (options.markerStyle?.visible !== false) {
        ...
        if (options.markerStyle?.icon) {
          if (typeof options.markerStyle.icon === 'string') {
            markerOptions.icon = {
              url: options.markerStyle.icon
            };
          } else {
            // 华为地图的图标处理方式
            markerOptions.icon = {
              url: options.markerStyle.icon.url,
              scale: options.markerStyle.icon.size ? 
                Math.min(options.markerStyle.icon.size[0], options.markerStyle.icon.size[1]) / 32 : 1
            };
          }
        }
  ...
  animateTimeBasedPathSync(map: any, options: ITimeBasedPathAnimateOptions): ITimeBasedPathAnimationController {
    // 初始化配置
    const config = {
      speed: options.speed || 1.0,
      autoPlay: options.autoPlay || false,
